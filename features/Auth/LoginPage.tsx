...
import { Shield, Mail, Lock, ArrowRight, Chrome, Info, User } from 'lucide-react';
...
export const LoginPage = () => {
    ...
    if (isRegister) {
      try {
        ...
        if (data?.user?.identities?.length === 0) {
          setRegisterNotice('Este e-mail já está cadastrado. Faça login.');
        } else {
          setRegisterNotice('Enviamos um e-mail de confirmação para sua caixa de entrada.');
        }

        setShowRegisterSuccess(true);
      } catch (err: any) {
        console.error('Erro ao criar conta:', err);
        alert(`Falha ao criar conta: ${err?.message || 'Erro desconhecido'}.`);
      } finally {
        setIsLoading(false);
      }
      return;
  ...
  const handleGoogleLogin = async () => {
    if (!isSupabaseConfigured) {
      alert("⚠️ Modo de Desenvolvimento\n\nConfiguração do Supabase ausente ou inválida. Use o login por e-mail (mock) para testar.");
      return;
    }
    ...
    try {
      ...
      // Note: In preview environments, the redirect might not happen or might be blocked.
      // The user remains on the page in that case.
    } catch (err: any) {
      console.error("Erro no login Google:", err);
      alert(`Falha no login com Google: ${err?.message || 'Erro desconhecido'}. \n\nSe estiver no ambiente de Preview, use o login por e-mail.`);
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#0b0f19] text-white flex flex-col items-center justify-center p-6 relative overflow-hidden">
       ...
       <div className="w-full max-w-md relative z-10">
          ...
          <div className="flex flex-col items-center mb-8">
             ...
             <p className="text-gray-400 font-medium">Gestão de Acampamento</p>
          </div>
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-[2.5rem] shadow-2xl animate-in fade-in slide-in-from-bottom-4">
             ...
             {showRegisterSuccess ? (
               <div className="text-center">
                 ...
                 <h2 className="text-xl font-black text-white">Cadastro concluído</h2>
                 <p className="mt-2 text-sm text-gray-400">
                   {registerNotice || 'Sua conta foi criada. Verifique seu e-mail para confirmar e depois faça login.'}
                 </p>
                 <button
                   type="button"
                   onClick={() => {
             ...
             <div className="flex gap-4 mb-8 border-b border-white/10 pb-4">
                <button 
                  ...
                  className={`flex-1 pb-2 text-sm font-bold uppercase tracking-widest transition-colors ${!isRegister ? 'text-cyan-400 border-b-2 border-cyan-400' : 'text-gray-500 hover:text-gray-300'}`}
                >
                  Entrar
                </button>
                <button 
          ...
          <div className="mt-8 text-center">
             ...
             >
                ...
                <span className="text-cyan-400 font-bold underline">{isRegister ? 'Faça Login' : 'Cadastre-se'}</span>
             </button>
             
             <p className="text-[10px] text-gray-700 mt-4 flex items-center justify-center gap-1">
                <Info size={10} /> Segurança garantida por Supabase Auth
             </p>
          </div>
       </div>
    </div>
