...
import { ArrowLeft, Target, Calculator, Plus, Trash2, Save, X, Edit2, AlertTriangle, CheckSquare, Calendar, CheckCircle, Info } from 'lucide-react';
...
export const OrgForecast = () => {
  ...
  const potentialRevenue = people.reduce((acc, p) => acc + p.totalPrice, 0);
  const realizedRevenue = people.reduce((acc, p) => acc + p.amountPaid, 0);
  const currentPax = people.length;
  const paidPax = people.filter(p => p.amountPaid >= p.totalPrice).length;

  const TARGET_PAX = 60;
  const TARGET_TICKET = 430;
  const TARGET_REVENUE = TARGET_PAX * TARGET_TICKET;
  ...
  const realizedExpenses = transactions.filter(t => t.type === 'SAIDA').reduce((acc, t) => acc + t.amount, 0) + fixedCostRent;

  useEffect(() => {
    const chacaraProj = expenseProjections.find(p => p.categoryMapping === 'ALUGUEL_CHACARA');
    if (!chacaraProj) return;
    if (fixedCostRent !== chacaraProj.amount) {
      updateFixedCostRent(chacaraProj.amount);
    }
  }, [expenseProjections, fixedCostRent, updateFixedCostRent]);
  ...
  const confirmExecution = async () => {
    ...
    try {
      ...
      console.error('Erro ao lancar no caixa:', error);
  ...
  const undoExecution = async (item: ProjectionItem) => {
    try {
      ...
      console.error('Erro ao desfazer lancamento:', error);
      ...
      <div className="bg-gray-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-3 gap-10 items-center">
          ...
          <div className="text-center lg:text-left relative">
            ...
            <p className="text-xs text-gray-400">Total previsto na tabela de despesas detalhadas.</p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="p-6 bg-white border border-gray-100 flex flex-col justify-between h-full">
            ...
            <div className="space-y-4">
              <div className="relative pt-2">
                ...
                <div className="w-full bg-gray-100 h-4 rounded-full overflow-hidden relative">
                  <div className="h-full rounded-full bg-emerald-500 transition-all duration-1000" style={{ width: `${revenueProgress}%` }} />
                </div>
              </div>

              <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 space-y-3">
                <div className="flex justify-between items-center border-b border-gray-200 pb-2">
                  <span className="text-xs text-gray-500 font-bold uppercase">Projecao Inicial ({TARGET_PAX}x {TARGET_TICKET})</span>
                ...
                <div className="flex justify-between items-center">
                  <span className="text-xs text-gray-500 font-bold uppercase">Projecao Atual ({currentPax}x 432)</span>
                ...
                <div className="flex justify-between items-center pt-2 border-t border-gray-200">
                  <span className="text-xs text-red-500 font-bold uppercase">Deficit sobre Meta</span>
          ...
          <div className="mt-4 flex items-start gap-2 bg-yellow-50 p-3 rounded-xl border border-yellow-100">
            ...
            <p className="text-xs text-yellow-800 leading-tight">
              <span className="font-bold">Atencao:</span> O deficit de inscritos ({TARGET_PAX - currentPax} a menos) deve ser compensado com ajuste nas despesas variaveis.
            </p>
          </div>
        </Card>
        ...
        <Card className="p-6 bg-blue-50 border-blue-100 flex flex-col justify-center">
          <h4 className="font-black text-blue-900 mb-2 text-lg flex items-center gap-2">
            <CheckSquare size={18} /> Transferencia para o Caixa
          </h4>
          <p className="text-sm text-blue-700 leading-relaxed mb-4">
            Use <strong className="bg-white px-2 py-0.5 rounded border border-blue-200 text-blue-600">Revisar</strong> para confirmar e lancar no caixa oficial.
          </p>
      ...
      <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-xl overflow-hidden">
        <div className="p-8 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <div>
            ...
            <p className="text-gray-400 text-xs font-bold mt-1">Tabela integrada ao Caixa e ao Estoque.</p>
          </div>
          <button onClick={() => setIsAdding(true)} className="bg-black text-white px-4 py-2 rounded-xl font-bold text-xs flex items-center gap-2 hover:scale-105 transition-transform">
